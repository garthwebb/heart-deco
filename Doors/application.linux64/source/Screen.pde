class Screen {
  ...
  void setHorzLeft(int pos, color col) {
    int[] pair = horzLineMap[pos];
    ledVal[0][pair[0]] = col;
    ledVal[1][pair[1]] = col;
  }
  
  void setHorzRight(int pos, color col) {
    int[] pair = horzLineMap[pos];
    ledVal[2][pair[0]] = col;
    ledVal[3][pair[1]] = col;
  }
  
  void setVertLeft(int pos, color col) {
  ...
  void setVertRight(int pos, color col) {
    int[] pair = vertLineMap[pos];
    ledVal[2][pair[0]] = col;
    ledVal[3][pair[1]] = col;
  ...
  void decayColor(int decay) {
    decayColor(decay, 5);
  }
  
  void decayColor(int decay, int colorShift) {
    for (int strip=0; strip<4; strip++) {
      for (int led=0; led<240; led++) {
        color curCol = ledVal[strip][led];
  ...
  void decayToRed(float decay, float colorShift) {
    for (int strip=0; strip<4; strip++) {
      for (int led=0; led<240; led++) {
        color curCol = ledVal[strip][led];
        float bright = brightness(curCol);
